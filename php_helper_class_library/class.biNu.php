	...
	private function get_link() {
		if (count($this->link_area) > 0) {
			...
			foreach ($this->link_area as $link_section) {
				$the_link_area .= '		<link url="'.$link_section['url'].'" x="5" y="y+5" w="width" mode="truncate"'.'" align="'.' style="'.$link_section['style']. 'linkType="o"'.'"><text  w="width-10" mode="wrap" mode="wrap" style="'.$link_section['style'].'">'.trim($link_section['link']).'</text></link>'.PHP_EOL;
			}
			$the_link_area .= '	</fixed>'.PHP_EOL;
